...
$rec_isis_len=800000;  //PHYSICAL MAX SIZE OF EACH RECORD
...
if ($debug) echo "converter=$converter_path.<p>";
$tipo=$arrHttp["Tipo"];
$dbname=$arrHttp["base"];
...
if ($debug) echo "upload_max_filesize = ".$max."<br>";
//var_dump($files);
foreach ($files['name'] as $key=>$name)
{
//echo "size=" . $files['size'][$key] .'<p> ';
//die;
...
//."/";
			...
			$s=explode("/" ,$destpath);
			$ix=count($s);
			$destpath =  $s[$ix-3] . '/' . $s[$ix-2];
			if ($destpath==$dbname)
				$destpath="";
			else
				$destpath=$destpath.'/';
			if ($debug) echo 'dest_path='.$destpath. '// storein='. $storein . "<br>";
...
//			echo $res."<p>" ;
			...
			if ($target=='linux') $converter_path = str_replace('\\', '/', $converter_path);
			...
			if ($debug) echo  "converter path:".$converter_path . "<BR>";
			switch($tipo){
			case "B":
				if ($target=='pc') $converter_path = str_replace('/', '\\', $converter_path);
				//echo "mx_path=". $mx_path0 . "<BR>";
				$destfilename = $name;
				if ($debug) echo "destfilename=".$destfilename."<BR>";
				$s=explode("." ,$name);
				$ix=count($s)-1;
				$name = '';
                		for ($i=0; $i<$ix; $i++)
...
//				echo $converterpdf."  ".$s[$ix];
				...
				$xcmd = $converter.$files['tmp_name'][$key];
//."  ".$redir.$name;
				if ($debug) echo $xcmd."<BR>";
//				echo "<hr>";
				//die;
				$content=array();
				$res=exec($xcmd,$content,$result);
				if ($debug) echo "<p>result: $result<p>";
				if ($result==0){
					$texto="";
...
//						echo "<font color=red>$value</font><br>";
						...
						if ($seq==1)   {
//							$ValorCapturado.= "<proc><pft>cat(delflds.prc)</pft><proc>";
              ...
              $ValorCapturado.="<".$Tag.">".$value."</".$Tag.">";
             	$ValorCapturado=urlencode($ValorCapturado);
						...
						$Formato="/abcd/www/bases/dubcore/pfts/en/dubcore";
						...
						foreach ($ValorCapturado as $value) echo "valorcapturado=$value\n";
					}
				}
				break;
		}
