...
?>	<a class="btn btn-success" href="javascript:SendForm()" class="defaultButton saveButton">		<span class="glyphicon glyphicon-floppy-disk"></span> <?php echo $msgstr["SAVE"]?>
	</a>
...
function DisplayProfiles(){global $db_path,$msgstr,$encabezado;
	...
	foreach ($fp as $val){
		...
		if ($val!=""){
			$p=explode('|',$val);
            if ($p[0]!="adm"){
				echo "<tr><td>".$p[1]." (".$p[0].")</td><td><a class=\"btn btn-primary\" href=profile_edit.php?profile=".$p[0]."$encabezado&Opcion=edit><span class=\"glyphicon glyphicon-pencil\"></span> ".$msgstr["EDIT"]."</a>  ";
				?>
				<a class="btn btn-danger" href="javascript:DeleteProfile('<?php echo $p[0]; ?>')"><span class="glyphicon glyphicon-remove"></span> <?php echo $msgstr["delete"]; ?></a></td>
				<?php
			}
		}
	}
...
?>	 			 		<div class="alert alert-danger" role="alert"><?php echo $msgstr["INUSE"];?></div>
<?php	
	 		return;	 	}
	}
    $fp=file($db_path."par/profiles/profiles.lst");
	...
	foreach ($fp as $dbs){
		...
		if ($dbs!=""){			$dd=explode('|',$dbs);
			...
			if ($dd[0]!="acces" and $dd[0]!="suggestions" and $dd[0]!="purchaseorder" and $dd[0]!="copies"
				...
				echo "<th width=33%>".$msgstr["DATABASES"]."</th><th width=33%>".$msgstr["DISPLAYFORMAT"]."</th><th width=33%>".$msgstr["WORKSHEET"]."</TH>";
				if ($inicio=="S"){					$inicio="N";
				...
				foreach ($profile_usr as $key=>$value){					$value=trim($value);
					if (substr($key,0,7)=="CENTRAL"){
						...
						if ($k[1]=="CRDB" or $k[1]=="TRANSLATE"  or $k[1]=="USRADM" or $k[1]=="EDHLPSYS" or $k[1]=="EXDBDIR"){
							continue;						}
						if ($i>2){
							echo "<tr>";
							$i=0;
