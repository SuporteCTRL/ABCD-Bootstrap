...
class marc
{
        ...
        function setRecords($identifier, $path)
	{
            global $BVS_CONF, $configurator, $isisBroker, $MARC_TAG_NAME;
                ...
                //create an xml to acess data through wxis-modules
		...
		$xmlparameters .= "</parameters>\n";
                ...
                //xml returned from wxis-modules
		...
		while ($posicion1>0)
		{
                    //put data from xml in an array
                    $elemento = substr($rawxml,$posicion1,$posicion2-$posicion1+9); //elemento is a string piece
                    $record = new Record(); //new array
                    $record->unserializeFromString($elemento); //put on "record" array, all xml elements
                    $tempField = $record->campos; //catch in "record" array, the element "campos"
                    $tempRecord = array();
